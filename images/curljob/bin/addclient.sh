...
curl -s -X POST -H "content-type: application/json;charset=utf-8" -H "Authorization: Bearer ${ACCESS_TOKEN}" -d '{"client_id":"'${OAUTH_CLIENT_ID}'","client_secret":"'${OAUTH_CLIENT_SECRET}'","name":"'${OAUTH_CLIENT_ID}'","scope":["openid","cloud_controller.read","tap.user"],"authorized_grant_types":["authorization_code"],"redirect_uri":["'${REDIRECT_URI}'"],"autoapprove":"true"}' ${UAA_ADDRESS}/oauth/clients
echo "exit code "$?
